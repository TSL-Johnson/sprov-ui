...
update() {
    ...
    install_base
    bash <(curl -L -s https://github.com/moranchenxi/sprov-ui/raw/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            restart
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/moranchenxi/sprov-ui/issues${plain}"
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/moranchenxi/sprov-ui/raw/master/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
